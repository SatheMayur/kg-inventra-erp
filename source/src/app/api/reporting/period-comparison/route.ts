...
import { getKolkataMonthBounds } from '@/lib/date-utils';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { thisMonthStart, lastMonthStart, lastMonthEnd } = getKolkataMonthBounds();
    ...
    const [
      ...
      db.request.count({ where: { status: { in: ['Issued', 'COMPLETED'] }, issuedAt: { gte: thisMonthStart } } }),
      db.request.count({ where: { status: { in: ['Issued', 'COMPLETED'] }, issuedAt: { gte: lastMonthStart, lte: lastMonthEnd } } }),
